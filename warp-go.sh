...
# 清理函数
cleanup_resources() {
  [ -e /opt/warp-go/monitor.pause ] && rm -f /opt/warp-go/monitor.pause
...
check_install() {
  if [ -s /opt/warp-go/warp.conf ]; then
    ...
    STATUS=0
    {
      ...
      latest=${latest:-'1.0.8'}
      wget --no-check-certificate -T5 -qO- https://raw.githubusercontent.com/666speed/warp-auto-reconnect/main/warp-go/warp-go_"$latest"_linux_"$ARCHITECTURE".tar.gz | tar xz -C /tmp/ warp-go
      chmod +x /tmp/warp-go
    }&
  fi
}
...
# 创建并管理 WARP 健康监控。它按 warp.conf 当前 AllowedIPs 自动判断检测 IPv4、IPv6 或双栈。
create_monitor() {
  mkdir -p /opt/warp-go/ >/dev/null 2>&1
  cat > /opt/warp-go/warp-go-monitor.sh << 'EOF'
#!/usr/bin/env bash
...
log_msg() {
  ...
  printf '%s %s\n' "$(date '+%F %T')" "$*" >> "$LOG_FILE"
  if [ -s "$LOG_FILE" ] && [ "$(wc -c < "$LOG_FILE" 2>/dev/null)" -gt 1048576 ]; then
    tail -n 200 "$LOG_FILE" > "${LOG_FILE}.tmp" 2>/dev/null && mv -f "${LOG_FILE}.tmp" "$LOG_FILE"
  fi
}

want_ipv4() {
  grep -qsE '^[#[:space:]]*AllowedIPs[[:space:]]*=.*0\.0\.0\.0/0' "$CONF"
}

want_ipv6() {
  grep -qsE '^[#[:space:]]*AllowedIPs[[:space:]]*=.*::/0' "$CONF"
}
...
restart_warp() {
  ...
  if command -v systemctl >/dev/null 2>&1 && [ -s /lib/systemd/system/warp-go.service ]; then
    ...
    sleep 1
    /opt/warp-go/warp-go --config=/opt/warp-go/warp.conf >/dev/null 2>&1 &
  fi
}
...
EOF
  chmod +x /opt/warp-go/warp-go-monitor.sh

  if echo "$SYSTEM" | grep -qvE "Alpine|OpenWrt"; then
    cat > /lib/systemd/system/warp-go-monitor.service << EOF
[Unit]
Description=warp-go health monitor
After=network-online.target warp-go.service
Wants=network-online.target warp-go.service
...
monitor_stop() {
  rm -f /opt/warp-go/monitor.enable /opt/warp-go/monitor.pause
  if echo "$SYSTEM" | grep -qvE "Alpine|OpenWrt"; then
    systemctl disable --now warp-go-monitor >/dev/null 2>&1
  else
...
monitor_pause() {
  [ -d /opt/warp-go ] && touch /opt/warp-go/monitor.pause
}

monitor_resume() {
  if [ -e /opt/warp-go/monitor.enable ]; then
...
uninstall() {
  ...
  rm -rf /opt/warp-go /lib/systemd/system/warp-go.service /lib/systemd/system/warp-go-monitor.service /usr/bin/warp-go /tmp/warp-go*
...
ver() {
  ...
  wget -T2 -O /tmp/warp-go.sh https://raw.githubusercontent.com/666speed/warp-auto-reconnect/main/warp-go.sh
  if [ -s /tmp/warp-go.sh ]; then
    if [ "$WARP_GO_FORCE_UPDATE" != 1 ] && ! grep -q 'CUSTOM WARP-GO HEALTH MONITOR' /tmp/warp-go.sh && grep -q 'CUSTOM WARP-GO HEALTH MONITOR' "$0" 2>/dev/null; then
      rm -f /tmp/warp-go.sh
      warning " 下载的上游脚本不包含自动重连补丁，已保留当前二改脚本。后期更新请把新版源码给我合并；如必须强制覆盖，可执行 WARP_GO_FORCE_UPDATE=1 warp-go v "
      exit 1
    fi
...
net() {
  ...
  info " $(text_eval 25) "
  [ -s /opt/warp-go/warp-go ] && [ ! -x /opt/warp-go/warp-go-monitor.sh ] && create_monitor
  monitor_start
